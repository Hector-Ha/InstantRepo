...
- Public CLI mirror roadmap issue: #34. Implementation slices: #35 CLI foundation/app-data isolation, #36 repository setup commands, #37 Env Draft commands, #38 installed repos/diagnostics, #39 Env Vault commands, #40 settings/bridge metadata, #41 docs/private QA convention.
...
- CLI should expose `--version --json` or equivalent version metadata with app version, git commit when available, and CLI contract version. Breaking JSON contract changes should bump the CLI contract version so agents and private QA can detect drift.
...
- App data directory override must resolve to an absolute path, create the directory if missing, and reject dangerous targets such as drive root, home dir, repo root, or a path inside the target repo being analyzed.
